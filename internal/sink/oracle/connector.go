...
// Connector implements the sink.Connector interface for Oracle.
...
// New creates a new Oracle sink connector.
func New() *Connector {
	return &Connector{
...
func (c *Connector) Initialize(ctx context.Context, config sink.Config) error {
	...
	log.Info("Oracle sink initialized",
...
// SupportsDDL returns true (Oracle supports DDL events).
func (c *Connector) SupportsDDL() bool {
	return true
}

// ApplyDDL applies a DDL event to Oracle.
...
// SupportsTransaction returns true (Oracle supports transactions).
...
// writeDataEvent writes a data event to Oracle.
...
// getSchemaTable returns properly quoted schema and table name using Oracle
// double-quote identifier convention: "SCHEMA"."TABLE".
...
// getSchemaTableFromEvent returns the quoted schema.table from a ChangeEvent.
func (c *Connector) getSchemaTableFromEvent(e *event.ChangeEvent) string {
	return c.getSchemaTable(e.Table.Schema, e.Table.Table)
}

// executeInsert executes an INSERT statement with Oracle-style bind parameters (:1, :2, ...).
...
// executeUpdate executes an UPDATE statement with Oracle-style bind parameters.
...
// executeDelete executes a DELETE statement with Oracle-style bind parameters.
...
// buildDSN builds an Oracle DSN string using net.JoinHostPort for IPv6 compatibility.
// Format: oracle://user:password@host:port/service_name
func buildDSN(cfg *Config) string {
	hostPort := net.JoinHostPort(cfg.Host, strconv.Itoa(cfg.Port))
	return fmt.Sprintf("oracle://%s:%s@%s/%s",
		cfg.User, cfg.Password, hostPort, cfg.ServiceName)
}
